...
> **Purpose**: A comprehensive reference guide for presenting and explaining the **Predictive Retail Analytics Platform**. It provides simple non-technical analogies, deep mathematical & algorithmic explanations, detailed definitions for **every metric** across all 3 pages of the web app, a step-by-step **Render deployment guide**, and **30+ interview Q&As**.

---

## 📑 Table of Contents
1. [Project Overview & 30-Second Elevator Pitch](#1-project-overview--30-second-elevator-pitch)
2. [Data Engineering Pipeline](#2-data-engineering-pipeline)
3. [Machine Learning Model (SARIMA)](#3-machine-learning-model-sarima)
4. [Inventory Optimization Engine](#4-inventory-optimization-engine)
5. [Deep Dive: All Dashboard Pages & Metrics Explained](#5-deep-dive-all-dashboard-pages--metrics-explained)
   - [Page 1: Executive Dashboard](#page-1-executive-dashboard)
   - [Page 2: Demand Forecasting Engine (Actual vs. Forecast)](#page-2-demand-forecasting-engine)
   - [Page 3: Inventory Optimization Engine](#page-3-inventory-optimization-engine)
6. [Mock vs. Real Model Execution](#6-mock-vs-real-model-execution)
7. [Step-by-Step Render Deployment Guide](#7-step-by-step-render-deployment-guide)
8. [Comprehensive Interview Q&A (30+ Questions)](#8-comprehensive-interview-qa)
9. [Quick Reference Metrics Summary](#9-quick-reference-metrics-summary)
...
| `stores.csv` | 54 rows | `store_nbr`, `city`, `state`, `type`, `cluster` | Categorizes stores by geographical location, size, and store format (Type A–E). |
| `oil.csv` | ~1,218 rows | `date`, `dcoilwtico` | Daily WTI crude oil price. Critical because Ecuador's oil-dependent economy influences consumer purchasing power. |
| `holidays_events.csv` | ~350 rows | `date`, `type`, `locale`, `locale_name`, `description`, `transferred` | Tracks national, regional, and local holidays/events causing massive demand spikes. |
...
- **Original Series ADF Test**: $p\text{-value} = 0.3421$ (> 0.05) $\rightarrow$ **Non-stationary** (contains clear upward trend).
- **First Difference ($d=1$) ADF Test**: $p\text{-value} = 0.000002$ (< 0.05) $\rightarrow$ **Stationary** $\Rightarrow d=1$ was selected.
...
- *Optimal Order*: $\text{SARIMA}(1, 1, 1) \times (1, 1, 1)_7$ with minimum AIC score.
...
- $\sigma_d$: Standard deviation of daily demand for that product family.
- $L$: Replenishment Lead Time ($L = 7$ days).

### 2. Reorder Point ($ROP$)
The inventory threshold that automatically triggers a purchase order.

$$ROP = (\text{Average Daily Demand} \times L) + SS$$
...
- $D$: Annual Demand ($\text{Average Daily Demand} \times 365$).
- $S$: Fixed Order Cost ($50 per purchase order).
- $H$: Annual Holding Cost per unit ($25\% \text{ of } \$10 \text{ unit cost} = \$2.50/\text{year}$).
...
- **Calculation**: Sum of all $SS_i$ across all 33 families ($SS_i = 1.65 \cdot \sigma_i \cdot \sqrt{7}$).
...
$$\text{Total Risk} = \sum (\text{Stockout Units} \times \$15) + \sum (\text{Overstock Units} \times \$2)$$

  - *Stockout Units*: Days where 7-day rolling demand exceeded the Reorder Point. Penalty = $15/unit (lost sale & customer dissatisfaction).
  - *Overstock Units*: Days where demand fell below 50% of expected lead-time demand. Penalty = $2/unit (storage, depreciation, spoilage).
...
- **Calculation**: Sum of mean daily sales across all product categories ($\sum \mu_i$).
...
- **Light Blue Shaded Band**: 95% Confidence Interval ($\text{Forecast} \pm 1.96 \cdot \text{Std Error}$).
...
- Displays daily percentage error ($|\text{Actual} - \text{Forecast}| / \text{Actual}$).
- **Color-Coding**: Green (< 5% error), Amber (5–10% error), Red (> 10% error). Dashed horizontal line marks the 7.9% MAPE benchmark.
...
#### 1. Forecast MAPE (`🎯 X.X%`)
- **Calculation**: Computes real-time MAPE between the forecast line and the actual demand line for the selected horizon:

$$\text{MAPE} = \frac{1}{H} \sum_{h=1}^{H} \left| \frac{\text{Actual}_h - \text{Forecast}_h}{\text{Actual}_h} \right| \times 100$$
...
$$\bar{Y}_{\text{forecast}} = \frac{1}{H} \sum_{h=1}^{H} \hat{Y}_h$$

#### 3. Avg Actual Demand (`📈 X,XXX units/day`)
- **Calculation**: Mean of ground truth / actual demand over the same horizon:

$$\bar{Y}_{\text{actual}} = \frac{1}{H} \sum_{h=1}^{H} Y_h$$

#### 4. Peak Actual Demand (`🔺 X,XXX units`)
- **Calculation**: Maximum single-day sales value observed during the forecast horizon window ($\max(Y_h)$). Helps warehouse managers plan for peak capacity days.

#### 5. Demand Shift (`🔄 ±X.X%`)
- **Calculation**: Percentage change between historical 90-day baseline average and forecasted daily average:

$$\text{Demand Shift} = \left( \frac{\bar{Y}_{\text{forecast}} - \bar{Y}_{\text{historical}}}{\bar{Y}_{\text{historical}}} \right) \times 100$$
...
- **Rose Red Dotted Line**: **Actual Demand** (simulated/ground truth demand during the forecast period).
...
- **Calculation**: Sum of Economic Order Quantities ($EOQ$) for all SKUs flagged for reorder (`🔴 URGENT` or `🟡 REORDER`).
...
- Grouped bar chart comparing **Current On-Hand Stock** (blue) against the computed **Reorder Point** (red) per product family. Highlights stock deficit areas visually.
...
1. **Latency & UX**: Fitting or performing full statistical inference on a 3M-row SARIMA model inside a Streamlit web callback takes 30–60 seconds per user click. Generating realistic seasonal signals ($Y = \text{base} + \text{trend} + \text{weekly} + \text{monthly} + \text{oil} + \text{holidays} + \text{noise}$) allows instant responsive interactions.
...
3. **Interpretability**: Every parameter ($p, d, q$) map to real economic behaviors (lags, trends, shock errors), making it transparent for business stakeholders.

### Q3: What is MAPE, and why did you choose it over RMSE/MAE?
**Answer**: MAPE (Mean Absolute Percentage Error) measures relative error percentage ($7.9\%$). Unlike MAE or RMSE which are scale-dependent (in raw sales units), MAPE allows us to compare forecasting accuracy across product families with vastly different volumes (e.g., Grocery vs. Appliances).

### Q4: How is Safety Stock calculated?
**Answer**: $SS = Z \times \sigma_d \times \sqrt{L}$. With $Z = 1.65$ (95% service level), standard deviation of daily sales $\sigma_d$, and replenishment lead time $L = 7$ days.

### Q5: How is the $4.61B Inventory Risk calculated?
**Answer**: It evaluates rolling 7-day sales against the Reorder Point. If sales exceed stock capacity, stockout cost = excess units $\times \$15$. If sales fall under 50% expected, overstock cost = surplus units $\times \$2$. The sum across all categories yields $4.61B.
...
| Model Architecture | SARIMA(1,1,1)×(1,1,1)₇ | Weekly seasonality ($m=7$), first difference ($d=1$) |
