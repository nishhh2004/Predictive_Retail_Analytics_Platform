...
1. train.csv (~3,000,888 records)
   - Key Fields: id, date, store_nbr, family, sales, onpromotion
   - Purpose: Historical daily sales per product family per store. Target variable is 'sales'.

2. stores.csv (54 rows)
   - Key Fields: store_nbr, city, state, type, cluster
   - Purpose: Categorizes stores by geographical location, size, and store format (Type A to E).

3. oil.csv (~1,218 rows)
   - Key Fields: date, dcoilwtico
   - Purpose: Daily WTI crude oil price. Critical because Ecuador's oil-dependent economy influences consumer purchasing power.

4. holidays_events.csv (~350 rows)
   - Key Fields: date, type, locale, description, transferred
   - Purpose: Tracks national, regional, and local holidays causing massive demand spikes.

5. transactions.csv (~83,488 rows)
   - Key Fields: date, store_nbr, transactions
   - Purpose: Total customer foot traffic per store per day.

6. test.csv (~28,512 rows)
   - Key Fields: id, date, store_nbr, family, onpromotion
   - Purpose: Unlabeled test set for Kaggle benchmark validation.
...
- Product Families: 33 (Product categories like Grocery, Beverages, etc.)
...
- Inventory Risk: $4.61B (Combined historical stockout at $15/unit & overstock at $2/unit)
